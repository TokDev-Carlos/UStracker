...
from .branding import available_assets
...
DEFAULT_BRAND = {
    ...
    'theme_primary':'#155eef',
    'theme_accent':'#0f9f6e',
}


def projection_path(root: Path) -> Path:
    p=Path(root)/'UserData'/'Public'/'production'/'view.json'
...
def rebuild(root:Path, db:Database) -> dict:
    ...
    allowed={'company_display_name','contact_phone','contact_email','address_display','theme_primary','theme_accent'}
    brand={**DEFAULT_BRAND,**{k:v for k,v in settings.items() if k in allowed}}
    brand['assets']=available_assets(root)
...
def read(root:Path) -> dict:
    ...
    if not path.exists():
        data={'brand':{**DEFAULT_BRAND,'assets':available_assets(root)},'clients':[],'catalog':[],'integrations':{'drive':'PREPARED_DISABLED','tracking':'PREPARED_DISABLED','fiscal_official':'PREPARED_DISABLED'}}
